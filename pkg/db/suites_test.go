...
func TestDynamicSuitePatternMatching(t *testing.T) {
	tests := []struct {
		...
		{
			name:          "lp-interop pattern with product and suffix",
			suiteName:     "lp-interop-ACS--my-tests",
			shouldMatch:   true,
			patternReason: "matches ^lp-interop- pattern",
		},
		{
			name:          "lp-interop pattern simple",
			suiteName:     "lp-interop-Foo",
			shouldMatch:   true,
			patternReason: "matches ^lp-interop- pattern",
		},
		{
			name:          "lp-interop pattern minimal",
			suiteName:     "lp-interop-X",
			shouldMatch:   true,
			patternReason: "matches ^lp-interop- pattern",
		},
		{
		...
		{
			name:          "no pattern match - missing hyphen",
			suiteName:     "lp-interop",
			shouldMatch:   false,
			patternReason: "pattern requires characters after lp-interop-",
		},
		{
			name:          "no pattern match - dash prefix",
			suiteName:     "-lp-interop",
			shouldMatch:   false,
			patternReason: "pattern anchored to start",
		},
	}
