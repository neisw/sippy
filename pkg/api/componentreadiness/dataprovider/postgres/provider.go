...
func (p *PostgresProvider) queryTestStatus(ctx context.Context, release string, start, end time.Time,
	...
	if err := p.dbc.DB.WithContext(ctx).Raw(testStatusQuery, release, start, end, release, start, end).Scan(&rows).Error; err != nil {
...
func (p *PostgresProvider) queryTestDetails(ctx context.Context, release string, start, end time.Time,
	...
	if err := p.dbc.DB.WithContext(ctx).Raw(testDetailQuery, release, start, end, release, start, end).Scan(&rows).Error; err != nil {
